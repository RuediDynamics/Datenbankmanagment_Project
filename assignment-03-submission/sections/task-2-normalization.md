...
![Normalisiertes logisches Datenmodell (≥ 3NF). Änderung ggü. Task 1: `rangeId` aus `ShotResult` entfernt, `atTarget` kontextuell aufgelöst (gestrichelt). PlantUML-Quelle: `assets/diagrams/logical-model-normalized.puml`]({{< meta assignment-03-path >}}/assets/diagrams/logical-model-normalized.svg){#fig-logical-normalized}
